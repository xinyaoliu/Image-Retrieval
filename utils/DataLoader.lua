...
-- TODO: line 170 看question 的格式，写lable
...
function DataLoader.create(data_dir, batch_size, opt, mode)
    ...
    -- local train_questions_file = path.join(data_dir,'annotations/captions_train2014.json')
    -- local train_annotations_file = path.join(data_dir,'annotations/captions_train2014.json')

    -- local val_questions_file = path.join(data_dir, 'annotations/captions_val2014.json')
    -- local val_annotations_file = path.join(data_dir, 'annotations/captions_val2014.json')
    ...
    --     print('one-time setup: preprocessing...')
        DataLoader.json_to_tensor(train_questions_file, val_questions_file,questions_vocab_file, answers_vocab_file, tensor_file)
    -- end

    print('Loading data files...')
    local data = torch.load('/home/liuxinyao/1/data/data.t7')
    ...
    self.q_max_length = 57
    self.q_vocab_mapping = torch.load('/home/liuxinyao/1/data/questions_vocab.t7')
    self.q_vocab_size = 0
    ...
    print('Loading train fc7 features from ' .. opt.train_fc7_file)  --这东西是由extract_fc7.lua产生的，我觉得，一样的dataset，可以的
    local fc7 = torch.load('/home/liuxinyao/prototype/data/train_fc7.t7')
    local fc7_image_id = torch.load('/home/liuxinyao/prototype/data/train_fc7_image_id.t7')
    local fc7_mapping = {}
    ...
    for i = 1, self.batch_size * self.batch_data.train.nbatches do
        if fc7_mapping[data.train[i]['image_id']] then
    ...
    local fc7 = torch.load('/home/liuxinyao/prototype/data/val_fc7.t7')
    local fc7_image_id = torch.load('/home/liuxinyao/prototype/data/val_fc7_image_id.t7')
    local fc7_mapping = {}
    for i, v in pairs(fc7_image_id) do
        fc7_mapping[v] = i
    end
    ...
    for i = 1, self.batch_size * self.batch_data.val.nbatches do
        if fc7_mapping[data.val[i]['image_id']] then
...
function DataLoader:next_batch(split)
    ...
    if split == 'train' then
        ...
        local question = nn.JoinTable(1):forward({self.batch_data.train.question:narrow(1, (self.train_batch_idx - 1) * self.batch_size + 1, self.batch_size * 0.4),self.batch_data.train.question:narrow(1, (self.train_batch_idx - 1) * self.batch_size + 1 + self.batch_size * 0.4 - 1, self.batch_size - self.batch_size * 0.4)})
        ...
        local image_lable = torch.Tensor(self.batch_size)

        for i=1, self.batch_size * 0.4 do
            image_lable[i] = 1
        end
        for i=self.batch_size * 0.4 + 1, self.batch_size do
            image_lable[i] = 0
        end
        ...
        return question, image, image_lable
    else
        if self.val_batch_idx - 1 == self.batch_data.val.nbatches then self.val_batch_idx = 1 end
        local question = nn.JoinTable(1):forward({self.batch_data.val.question:narrow(1, (self.val_batch_idx - 1) * self.batch_size + 1, self.batch_size * 0.4),self.batch_data.val.question:narrow(1, (self.val_batch_idx - 1) * self.batch_size + 1 + self.batch_size * 0.4 - 1, self.batch_size - self.batch_size * 0.4)})
...
function DataLoader.json_to_tensor(in_train_q, in_val_q,  out_vocab_q, out_vocab_a, out_tensor)
