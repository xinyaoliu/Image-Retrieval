...
function L1HingeEmbeddingCriterion:updateOutput(input,y)   -- do something here...
   -- self.output=input[1]:dist(input[2],1);    -- ||x1-x2|| _1
   temp_sub = torch.cmul(input[1]:dist(input[2],1),input[1]:dist(input[2],1))
   -- if y == -1 then
	 -- self.output = math.max(0,self.margin - self.output);
   self.output= torch.mul(temp_sub,s) + math.max(0, -temp_sub + b)
   end
   return self.output
end
...
function L1HingeEmbeddingCriterion:updateGradInput(input, y)    -- do something here...
