...
-- TODO: 1, 把文字和图片的feature转换成hashcode，
        -- 2, 把criterion加上，  √
        -- 3, 把lable加上lable要在辨别的时候加，之前加不行     √
        -- 4, 看print出来的idx是什么东西，再决定怎么算accuracy
        -- 5, loss是什么鬼  不是重点啊...
        -- 6, 明天捋一捋
        --先试一下不要sm
...
cmd:option('-savefile', 'rtv_2', 'Filename to save checkpoint to')
-- gpu/cpu
...
if string.len(opt.init_from) > 0 then    --Initialize network parameters from checkpoint at this path 优化的参数，不管
    ...
    protos.sm:add(nn.Linear(opt.rnn_size, 64))   --loader.batch_data.train.nbatches * loader.batch_size  解决中
...
feval_val = function(max_batches)
    ...
    for i = 1, n do
        ...
        imf = imf:type('torch.IntTensor'):max(2)
        ...
        for j = 1, opt.batch_size do
            if idx[j][1] == q_batch[j][1] then  -- a_batch
                count = count + 1
            end
        end
            -- smcount = 0
            -- for i = 1, prediction:size(2) do
            --     for k = 1, prediction:size(2) do
            --         if prediction[j][i] == imf[j][k] then
            --             smcount = smcount + 1
            --         end
            --     end
            -- end
            ...
            -- print("------------------------print idx------------------")
            -- print(idx[j][1])          --[torch.CudaTensor of size 200x1]
            -- print(imf[j][1]) 
            -- if smcount>30 then  -- a_batch
...
feval = function(x)
    ...
    loss = protos.criterion:forward({prediction, imf}, 1)  --测试
    ------------ backward pass ------------
    ...
    dloss = protos.criterion:backward({prediction, imf}, 1)  --a_batch  测试
