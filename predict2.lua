...
lstm_clones = {}
lstm_clones = utils.clone_many_times(checkpoint.protos.lstm, loader.q_max_length)  -- loader.q_max_length + 1  解决中
...
function predict( question_string)   -- 解决中 (input_image_path, question_string)
    ...
    for t = 1, loader.q_max_length-1 do  --loader.q_max_length 解决中
        local lst = lstm_clones[t]:forward{qf:select(1,t):view(1,-1), unpack(rnn_state[t-1])}
        rnn_state[t] = {}
        for i = 1, #init_state do table.insert(rnn_state[t], lst[i]) end
    end 
                                                        -- imf:view(1,-1)                 
    local lst = lstm_clones[loader.q_max_length ]:forward{qf:select(2, loader.q_max_length), unpack(rnn_state[loader.q_max_length-1])}
...
predict(opt.input_image_path, opt.question)   -- (opt.input_image_path, opt.question)解决中
